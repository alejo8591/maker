...
from maker.core.models import User, Group, Invitation, Perspective, Module, ModuleSetting, Page, PageFolder
from maker.core.administration.forms import PerspectiveForm, UserForm, PasswordForm, \
                                               GroupForm, PageForm, PageFolderForm, FilterForm, SettingsForm, PERMISSION_CHOICES
from maker.core.mail import EmailInvitation
from maker.core.decorators import module_admin_required, maker_login_required, handle_response_format

from maker.identities.models import ContactType
from maker.core.administration.forms import ContactSetupForm
import re
...
def perspective_edit(request, perspective_id, response_format='html'):
    ...
    admin_module = Module.objects.filter(name='maker.core')[0]
...
def perspective_delete(request, perspective_id, response_format='html'):
    ...
    # Don't let users delete their last perspective
    other_perspectives = Perspective.objects.filter(trash=False).exclude(id=perspective_id)
    admin_module = all_modules.filter(name='maker.core')[0]
...
def setup(request, response_format='html'):
    ...
    for module in modules:
        if module.name in ('maker.projects', 'maker.sales', 'maker.services'):
            module.major = True
        else:
            module.major = False
...
def settings_view(request, response_format='html'):
    ...
    # default perspective
    try:
        conf = ModuleSetting.get_for_module('maker.core', 'default_perspective')[0]
