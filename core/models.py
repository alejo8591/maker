...
class Group(InteractionEntity):
    ...
    def get_perspective(self):
        ...
        ids = []
        try:
            for setting in ModuleSetting.get_for_module('maker.core', name='default_perspective', group=self):
...
class User(InteractionEntity):
    ...
    def is_admin(self, module_name=''):
        ...
        try:
            ...
            pass
        if access or module_name == 'maker.core':
            return access
        else:
            return self.is_admin(module_name='maker.core')
    ...
    def get_perspective(self):
        ...
        ids = []
        try:
            for setting in ModuleSetting.get_for_module('maker.core', name='default_perspective', user=self):
                ids.append(long(setting.value))
            id = ids[0]
            perspective = get_object_or_404(Perspective, pk=id)
        except:
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        perspective.save()
                    ModuleSetting.set_for_module('default_perspective', perspective.id, 'maker.core', user=self)
        return perspective
