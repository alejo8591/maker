...
from maker.finance.forms import TransactionForm, LiabilityForm, AccountForm, EquityForm, AssetForm, \
...
from maker.finance.models import Transaction, Liability, Category, Account, Equity, Asset, Currency, Tax
from maker.finance.csvapi import ProcessTransactions
from maker.sales.models import Product, SaleOrder
from maker.sales.forms import dict_currencies
from maker.finance.helpers import convert
...
def index_accounts(request, response_format='html'):
    ...
    if not request.user.get_profile().is_admin('maker.finance'):
...
def currency_edit(request, currency_id, response_format='html'):
    ...
    if not request.user.get_profile().has_permission(currency, mode='w') \
        and not request.user.get_profile().is_admin('maker_finance'):
