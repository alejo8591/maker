...
from maker.sales.forms import SettingsForm, OrderForm, ProductForm, SaleStatusForm, UpdateRecordForm, \
...
from maker.core.decorators import maker_login_required, handle_response_format, module_admin_required
...
def ordered_product_view(request, ordered_product_id, response_format='html'):
    ...
    if not request.user.get_profile().has_permission(ordered_product) \
        and not request.user.get_profile().is_admin('maker.sales'):
...
@maker_login_required
@handle_response_format
@module_admin_required('maker.sales')
...
def lead_view(request, lead_id, response_format='html'):
    ...
    if not profile.has_permission(lead) \
        and not profile.is_admin('maker.sales'):
...
def settings_view(request, response_format='html'):
    ...
    if not request.user.get_profile().is_admin('maker.sales'):
