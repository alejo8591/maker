...
from maker.core.rendering import render_to_response, render_string_template, render_to_string
...
from maker.services.models import Ticket, TicketRecord, TicketStatus, TicketQueue, Service, \
                                     ServiceLevelAgreement, ServiceAgent
from maker.services.forms import SettingsForm, MassActionForm, TicketForm, TicketStatusForm, \
...
def status_edit(request, status_id, response_format='html'):
    ...
    if not request.user.get_profile().has_permission(status, mode='w') \
        and not request.user.get_profile().is_admin('maker_services'):
...
def status_add(request, response_format='html'):
    ...
    if not request.user.get_profile().is_admin('maker.services'):
