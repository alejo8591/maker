...
from maker.core.models import ModuleSetting
from maker.core.api.handlers import ObjectHandler
from maker.services.models import TicketStatus, Service, ServiceLevelAgreement, ServiceAgent, TicketQueue, Ticket, TicketRecord
from maker.services.forms import TicketForm, TicketStatusForm, TicketRecordForm, QueueForm, \
                                    ServiceForm, ServiceLevelAgreementForm, AgentForm 
from maker.services.views import _get_default_context
...
class TicketStatusHandler(ObjectHandler):
    ...
    def check_create_permission(self, request, mode):
        return request.user.get_profile().is_admin('maker.services')
...
class ServiceHandler(ObjectHandler):
    ...
    def check_instance_permission(self, request, inst, mode):
        return request.user.get_profile().has_permission(inst, mode=mode) \
                or request.user.get_profile().is_admin('maker_services')
