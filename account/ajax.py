...
from maker.core.models import Comment, Object, UpdateRecord, Tag
from maker.core.forms import TagsForm
from maker.core.ajax import converter
...
def attachment(request, object_id, update_id=None):
    ...
    try:
        ...
        if object_id:
            ...
            dajax.add_data({'target': 'div.attachment-block[object="%s"]'%object_id, 'content': object_markup}, 'maker.add_data')
        ...
        if update_id:
            ...
            update_markup = render_to_string(template,
                               ...
                               context_instance=RequestContext(request),
                               response_format='html')
            dajax.add_data({'target': 'div.attachment-record-block[object="%s"]'%update_id, 'content': update_markup}, 'maker.add_data')
...
def easy_invite(request, emails=None):
    ...
    dajax.add_data({'target': "div.easy-invite[emails='%s']"%(emails_original), 'content': invite_markup}, 'maker.add_data')
    return dajax.json()
